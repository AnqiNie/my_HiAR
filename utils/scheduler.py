...
class FlowMatchScheduler():
    ...
    def step(self, model_output, timestep, sample, to_final=False, target_timestep=None):
        """
        ODE denoising step.
        Args:
            model_output: model-predicted flow (v = noise - x0)
            timestep: current timestep [B] or [B*T]
            sample: current noisy sample [B*T, C, H, W]
            to_final: whether to jump directly to the final sigma
            target_timestep: target timestep [B] or [B*T] or scalar.
                If provided, jump to the sigma corresponding to target_timestep instead of timestep_id + 1.
                This is critical for large-step jumps (e.g., steps defined by denoising_step_list).
        """
        if timestep.ndim == 2:
        ...
        if target_timestep is not None:
            # Use the sigma corresponding to the specified target_timestep for large-step jumping
            if not isinstance(target_timestep, torch.Tensor):
                target_timestep = torch.tensor([target_timestep], device=model_output.device)
            if target_timestep.ndim == 0:
                target_timestep = target_timestep.unsqueeze(0)
            if target_timestep.ndim == 2:
                target_timestep = target_timestep.flatten(0, 1)
            # Unify device to avoid CPU/CUDA mismatch
            target_timestep = target_timestep.to(self.timesteps.device)
            target_id = torch.argmin(
                (self.timesteps.unsqueeze(0) - target_timestep.float().unsqueeze(1)).abs(), dim=1)
            sigma_ = self.sigmas[target_id].reshape(-1, 1, 1, 1)
        elif to_final or (timestep_id + 1 >= len(self.timesteps)).any():
            sigma_ = 1 if (
                self.inverse_timesteps or self.reverse_sigmas) else 0
        else:
            sigma_ = self.sigmas[timestep_id + 1].reshape(-1, 1, 1, 1)
        prev_sample = sample + model_output * (sigma_ - sigma)
        return prev_sample.type_as(sample)
